...
api = Namespace(
    "average_intensity", description="Calculate average intensity operations"
)

average_intensity_model = api.model(
    "AverageIntensityModel",
    {
        "image": fields.String(
            required=True,
...
average_intensity_response_model = api.model(
    "AverageIntensityResponseModel",
    {
        "average_intensity": fields.Float(
            description="Calculated average intensity", example=128.5
        )
    },
)
...
class AverageIntensity(Resource):
    ...
    @api.response(200, "Success", average_intensity_response_model)
    def post(self):
        ...
        return jsonify({"average_intensity": average_intensity})
